...
bool TherapyRouter::isActive() const {
    return config_.enableSubthreshold || config_.enableRMP || 
           config_.enablePIP || config_.enableSidebands || config_.enableBinaural;
}
...
StereoSample TherapyRouter::process(double baseFreq, float baseAmp) {
    ...
    if (config_.enableRMP) {
        float s = rmp_.process(baseFreq, amp);
        out.left += s;
        out.right += s;
    }
    
    if (config_.enablePIP) {
        float s = pip_.process(baseFreq, amp);
        out.left += s;
        out.right += s;
    }
    
    if (config_.enableSidebands) {
        float s = sidebands_.process(baseFreq, amp);
        out.left += s;
        out.right += s;
    }
    
    if (config_.enableBinaural) {
        StereoSample b = binaural_.process(baseFreq, amp);
        out.left += b.left;
        out.right += b.right;
    }
